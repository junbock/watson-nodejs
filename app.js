...
function entityInsert(params){
  var sql = 'UPDATE Entity SET what_col=?, who_col=?, when_col=?, why_col=?,where_col=?, how_col=? WHERE session_id=?';
  ...
  assistant.message(payload, function (err, data) {
    ...
    searchDocs.addDocs(data.result, function () {
      actions.testForAction(data.result, req.body.session_id).then(function (d) {
        ...
        var what='', who='', when='', why='', where='', how='';
        var is_full = false;
        connection.query(sql,sessionId,function(err,result){
          ...
          let params = [what,who,when,why,where,how,sessionId];
          console.log(params);
          entityInsert(params);
          
          
          if(what,who,when,why,where,how != null){
            is_full = true;
          }
...
app.get('/api/diary',(req,res)=>{
  ...
  let sql = 'SELECT dateTime, content, image FROM Diary WHERE user_id=? AND Date(dateTime)=? order by dateTime desc';
